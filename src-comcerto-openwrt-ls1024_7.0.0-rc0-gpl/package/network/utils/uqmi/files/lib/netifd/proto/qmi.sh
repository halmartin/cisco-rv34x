...
proto_qmi_setup() {
	...
	local device apn auth username password pincode delay modes pdptype profile dhcpv6 autoconnect plmn metric $PROTO_DEFAULT_OPTIONS
	local cid_4 pdh_4 cid_6 pdh_6
	local ip_6 ip_prefix_length gateway_6 dns1_6 dns2_6
	json_get_vars device apn auth username password pincode delay modes pdptype profile dhcpv6 autoconnect plmn metric $PROTO_DEFAULT_OPTIONS
	...
	[ -c "$device" ] || {
		logger -t mobile -p local0.error "The specified control device does not exist"
		proto_notify_error "$interface" NO_DEVICE
		proto_set_available "$interface" 0
		return 1
	}
	...
	[ -n "$plmn" ] && {
		...
		if [ "$plmn" = 0 ]; then
			...
			mcc=${plmn:0:3}
			mnc=${plmn:3}
			logger -t mobile -p local0.debug "Setting PLMN to $plmn"
		fi
		uqmi -s -d "$device" --set-plmn --mcc "$mcc" --mnc "$mnc" || {
			logger -t mobile -p local0.error "Unable to set PLMN"
	...
	if [ "$dataformat" = '"raw-ip"' ]; then
		...
		[ -f /sys/class/net/$ifname/qmi/raw_ip ] || {
			logger -t mobile -p local0.debug "Device only supports raw-ip mode but is missing this required driver attribute: /sys/class/net/$ifname/qmi/raw_ip"
			return 1
		}

		logger -t mobile -p local0.debug "Device does not support 802.3 mode. Informing driver of raw-ip only for $ifname .."
	...
	[ -n "$apn" ] || {
		...
		[ -z "$apn" ] && {
			logger -t mobile -p local0.error "$interface: No APN specified"
			proto_notify_error "$interface" NO_APN
			return 1
		}
	}

	logger -t mobile -p local0.debug "Starting network $interface APN=$apn"
	...
	[ "$pdptype" = "ip" -o "$pdptype" = "ipv4v6" ] && {
		...
		if ! [ "$cid_4" -eq "$cid_4" ] 2> /dev/null; then
			logger -t mobile -p local0.error "Unable to obtain client ID"
		...
		if ! [ "$pdh_4" -eq "$pdh_4" ] 2> /dev/null; then
			logger -t mobile -p local0.error "Unable to connect IPv4"
		...
		connstat=$(uqmi -s -d "$device" --get-data-status)
                [ "$connstat" == '"connected"' ] || {
                        logger -t mobile -p local0.error "No data link!"
                        uqmi -s -d "$device" --set-client-id wds,"$cid_4" --release-client-id wds
                        proto_notify_error "$interface" CALL_FAILED
                        return 1
		}
	...
	[ "$pdptype" = "ipv6" -o "$pdptype" = "ipv4v6" ] && {
		...
		if ! [ "$pdh_6" -eq "$pdh_6" ] 2> /dev/null; then
			logger -t mobile -p local0.error "Unable to connect IPv6"
	...
	logger -t mobile -p local0.debug "Setting up $ifname"
...
proto_qmi_teardown() {
	...
	logger -t mobile -p local0.debug "Stopping network $interface"
