...
getIfaceByIP(char *ip, char *interfaceName)
{
  struct ifaddrs *ifaddr, *ifa;
  struct in_addr ip_address, if_ipaddress, if_mask;
  ...
  for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
    ...
    if (family == AF_INET) {
      ...
      ip_address.s_addr = inet_addr(ip);
      ...
      if_ipaddress.s_addr = inet_addr(host);
      ...
      if_mask.s_addr = inet_addr(host);

      if (is_same_net(ip_address, if_ipaddress, if_mask))
      {
        printf("Falls into subnet of interface:%s\r\n", ifa->ifa_name);
...
{ 
  ...
  if (argc < 3 || argc > 4)
    { 
      ...
      fprintf(stderr, "usage: dhcp_release <addr> <mac> [<client_id>]\n");
      exit(1);
    }
  ...
  getIfaceByIP(argv[1], interfaceName);

  /* This voodoo fakes up a packet coming from the correct interface, which really matters for 
     a DHCP server */
  //printf("Final interface is:%s\r\n", interfaceName);
  ...
  server = find_interface(lease, nl, if_nametoindex(interfaceName));
  
  memset(&packet, 0, sizeof(packet));
 
  packet.hlen = parse_hex(argv[2], packet.chaddr, DHCP_CHADDR_MAX, &mac_type);
  if (mac_type == 0)
  ...
  *(p++) = sizeof(server);
  ...
  if (argc == 5 && strcmp(argv[3], "*") != 0)
    {
      unsigned int clid_len = parse_hex(argv[3], p+2, 255, NULL);
      *(p++) = OPTION_CLIENT_ID;
      *(p++) = clid_len;
      p += clid_len;
    }
