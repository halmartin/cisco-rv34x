...
################  Script Syntax  ###############
# rv16x_26x_fw_unpack.sh <firmware_file_name>.img  #
################################################
...
MTD_BAREBOX_ENV=15
MTD_ENV=2
BAREBOX=barebox-c2krv16x-26x.bin
...
if [ -f $IMG_FILE ]; then
  ...
  echo "Usage: rv16x_26x_fw_unpack.sh <*.img>"
  cd - > /dev/null 2>&1
  exit 11
fi

# Start Diag LED Blinking
rv16x_26x_led.sh diag slow > /dev/null 2>&1 &
...
if [ x$ACTIVE_IMG = ximage1 ]; then
  ...
  pgrep -f "rv16x_26x_led.sh diag slow" | xargs kill -9 > /dev/null 2>&1
  rv16x_26x_led.sh diag fast > /dev/null 2>&1 &
  cd - > /dev/null 2>&1
  exit 12
fi

# Calculate md5sum of IMG file
echo `md5sum $IMG_FILE | cut -d " " -f 1` > md5sum_fw-rv16x_26x-img

echo -n "Extracting files from $IMG_FILE ..."
dd if=$IMG_FILE of=rv16x_26x_fw.gz bs=64 skip=1 > /dev/null 2>&1
[ "$?" -ne 0 ] && {
        logger -t system -p local0.error "image extraction failed"
}

tar -xzvf rv16x_26x_fw.gz  > /dev/null 2>&1
[ "$?" -ne 0 ] && {
	logger -t system -p local0.error "Invalid firmware file."
}

# Deleting intermediate tar files to free /tmp memory
rm -rf rv16x_26x_fw.gz
echo "done."
...
MD5SUM_PKG="`cat md5sum_fw-rv16x_26x | cut -d " " -f 1`"
[ "$?" -ne 0 ] && {
	logger -t system -p local0.error "can't open md5sum_fw-rv16x_26x: No such file or directory."
  	echo "Firmware Update Failed!"
	pgrep -f "rv16x_26x_led.sh diag slow" | xargs kill -9 > /dev/null 2>&1
	rv16x_26x_led.sh diag fast > /dev/null 2>&1 &
	rm -rf $DIR_FIRMWARE* > /dev/null 2>&1
...
[ "$?" -ne 0 ] && {
        ...
        cd - > /dev/null 2>&1
	sleep 10
        pgrep -f "rv16x_26x_led.sh diag fast" | xargs kill -9 > /dev/null 2>&1
        exit 12
}
...
if [ ! -e "$ROOTFS" ]; then
 ...
 logger -t system -p local0.info "Erasing Stale USB dongle driver"
mv /mnt/packages/mobile /tmp/
...
cp -f firmware_version firmware_time img_version md5sum_fw-rv16x_26x-img $DIR_IMG > /dev/null 2>&1
echo "done."
