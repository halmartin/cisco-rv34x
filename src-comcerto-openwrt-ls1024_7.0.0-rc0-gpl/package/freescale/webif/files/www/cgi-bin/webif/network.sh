...
PPP_GENERIC_DEVICE="ppp+"
fw_device_cfg()
{
	local action=$1
	local zone=$2
	local device_name=$3
	local fw_zone_devices=""
	local fw_zone_index

	fw_zone_index=$(uci show firewall | grep -w "$interface" | grep zone | grep name | cut -d'[' -f 2 | cut -d']' -f 1)
	if [ -n "$fw_zone_index" -a $fw_zone_index -ge 0 ]; then
		fw_zone_devices=$(uci get firewall.@zone[$fw_zone_index].device)
		fw_zone_cfg_name=$(uci show firewall.@zone[$fw_zone_index] | grep "=zone" | cut -d'.' -f 2 | cut -d'=' -f 1)
		if [ "$action" = "add" ]; then
			uci_set firewall "$fw_zone_cfg_name" device "$fw_zone_devices $device_name"
		else # remove case
			fw_zone_devices=$(echo $fw_zone_devices | sed "s/$device_name//g")
			uci_set firewall "$fw_zone_cfg_name" device "$fw_zone_devices"
		fi
	fi
}
			...
			case "$FORM_proto" in
				pppoe|pppoa|pptp|3g)
					...
					if [ "$FORM_ppp_redial" = "persist" ]; then
						...
						[ "$FORM_proto" != "3g" ] && fw_device_cfg remove "$interface" "$PPP_GENERIC_DEVICE"
					else
						uci_remove "network" "$interface" "keepalive"
						uci_set "network" "$interface" "demand" "$FORM_demand"
						[ "$FORM_proto" != "3g" ] && fw_device_cfg add "$interface" "$PPP_GENERIC_DEVICE"
					fi
					uci_set "network" "$interface" "defaultroute" "$FORM_defaultroute"
					uci_set "network" "$interface" "ppp_redial" "$FORM_ppp_redial";;
				*)
					fw_device_cfg remove "$interface" "$PPP_GENERIC_DEVICE"
					;;
			esac
	        uci_set "network" "$interface" "ifname" "$FORM_ifname"
