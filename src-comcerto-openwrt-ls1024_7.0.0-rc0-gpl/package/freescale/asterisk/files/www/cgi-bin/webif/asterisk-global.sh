...
if empty "$FORM_save"; then
  ...
  config_get FORM_ipv6_mode general ipv6_mode
else
  SAVED=1
  validation_success=""
...
EOF
  ...
  equal "$?" 0 && {
    ...
    uci_set asterisk general ipv6_mode "$FORM_ipv6_mode"
    ...
    config_set general ipv6_mode "$FORM_ipv6_mode"
  }
fi
...
function checkcodecs()
{
  ...
  if ( ( document.forms[0].codec1.checked ) ||
       ...
       ( document.forms[0].codec14.checked ) )
  {
    no_code_selected = 0 ;
  }
  else
...
function selectall()
{
    ...
    document.forms[0].codec14.checked   = true ; 
...
function clearall()
{
    ...
    document.forms[0].codec14.checked   = false ; 
...
do
  ...
  case "$codec_name" in
    ...
    "OPUS")    alignement_spaces=$OPUS_SPACES   ;;   
  esac
  codecs_field="$codecs_field
...
display_form <<EOF
start_form|@TR<<Network configuration>>
field|@TR<<IPv6>>|ipv6_mode
radio|ipv6_mode|$FORM_ipv6_mode|enable|@TR<<Enable>>
radio|ipv6_mode|$FORM_ipv6_mode|disable|@TR<<Disable>>
helpitem|Activation
helptext|After changing this option Asterisk must be restarted
end_form
EOF
